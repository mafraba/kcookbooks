...
export KUBERNETES_RELEASE_VERSION=v0.15.0
export ETCD_VERSION=v2.0.10
export KUBERNETES_CLUSTER_ID=kube
...
export KUBERNETES_SLAVE_HOSTNAME=$KUBERNETES_CLUSTER_ID-slave-01
...
install_weave() {
  ...
  sudo curl --location https://github.com/zettio/weave/releases/download/latest_release/weave --output /opt/bin/weave
  sudo curl --location https://raw.github.com/errordeveloper/weave-demos/master/poseidon/weave-helper --output /opt/bin/weave-helper
  sudo chmod +x /opt/bin/weave
  sudo chmod +x /opt/bin/weave-helper
  # Load config, something like:
  ...
  /opt/bin/weave launch $WEAVE_PEERS
  # Start weave-helper
  /opt/bin/weave-helper $IFACE &
}
...
update_services_config() {
  ...
  if [[ $INSTALLER_TYPE == 'master' ]]; then
    ...
    echo -e "KUBE_CONTROLLER_MANAGER_OPTS=\"--address=0.0.0.0 --master=127.0.0.1:8080 --v=0 \"" | sudo tee -a /etc/default/kube-controller-manager
    ...
    echo "KUBELET_OPTS=\"--address=0.0.0.0 --port=10250 --hostname_override=$KUBERNETES_SLAVE_HOSTNAME --api_servers=http://$KUBERNETES_MASTER_HOSTNAME:8080 --enable_server=true --logtostderr=true --v=0\"" | sudo tee -a /etc/default/kubelet
